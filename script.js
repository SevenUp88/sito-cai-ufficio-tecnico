 ...
 * VERSIONE FINALE: Gestisce la ricerca e la visualizzazione con funzione ZOOM.
 */
document.addEventListener('DOMContentLoaded', () => {
    ...
    const fetchAllSearchableData = async () => { /* ... Il codice di questa funzione va qui ... */ };
    const handleSearch = () => { /* ... Il codice di questa funzione va qui ... */ };
    const displayResults = (results) => { /* ... Il codice di questa funzione va qui ... */ };
    ...
    if (detailsModalOverlay) {
        detailsModalOverlay.addEventListener('click', (e) => {
            if (e.target.closest('.modal-image-wrapper')) {
                const wrapper = e.target.closest('.modal-image-wrapper');
                let targetImage = wrapper.querySelector('#modal-image-attachments[style*="opacity: 1"], #modal-image-attachments:hover');
                if (!targetImage) targetImage = wrapper.querySelector('img[style*="display: block"]');
                if (targetImage && targetImage.src && !targetImage.src.includes('placeholder')) {
