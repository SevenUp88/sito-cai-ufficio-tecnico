...
async function loadThermolutzData() {
    if (!db) { console.error("Firestore non inizializzato!"); alert("Errore: Impossibile connettersi al database."); return Promise.reject("Firestore not initialized"); }
    try {
        ...
        if (thermolutzProducts.length === 0) { console.warn("Attenzione: Nessun dato prodotto Thermolutz trovato in Firestore."); }
        if (typeof updateDynamicSelectsBasedOnBrandAndPanel === "function") {
            updateDynamicSelectsBasedOnBrandAndPanel();
        }
        return Promise.resolve(); 
    } catch (error) { console.error(`Errore caricamento dati da Firestore (${FIRESTORE_RADIANT_COLLECTION_NAME}):`, error); alert(`Errore caricamento dati prodotti. Funzionalità limitate.`); return Promise.reject(error); }
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateRoomCardIdentifiers(cardElement, cardIndex) { /* ... (Implementazione COMPLETA come fornita nella risposta precedente) ... */
        ...
        ['.roomName', '.roomSurface', '.roomFlooring'].forEach(selector => {
            ...
            if (input) {
                ...
                if (cardElement.dataset.isNewlyAdded === 'true') {
        ...
        if (removeBtn) {
            ...
            newRemoveBtn.addEventListener('click', function() { cardElement.remove(); renumberAndSetupVisibleRoomCards(); });
        }
    }

    function renumberAndSetupVisibleRoomCards() { /* ... (Implementazione COMPLETA come fornita nella risposta precedente) ... */
        const visibleCards = roomsGridContainer.querySelectorAll('.room-card:not(.room-placeholder-template)');
        visibleCards.forEach((card, index) => { updateRoomCardIdentifiers(card, index + 1); });
    ...
    if(roomsGridContainer){ /* ... (Setup iniziale card come prima) ... */
        ...
        initialCards.forEach((card, index) => { updateRoomCardIdentifiers(card, index + 1); });
        updateTotalSurface(); 
    }
    ...
    if (addRoomBtn && roomsGridContainer) { /* ... (Listener addRoomBtn come prima) ... */
        addRoomBtn.addEventListener('click', function() {
            ...
            newRoomCard.classList.remove('room-placeholder'); newRoomCard.id = `roomCardAuto${nextRoomCardIdSuffix}`; newRoomCard.dataset.isNewlyAdded = 'true'; 
            updateRoomCardIdentifiers(newRoomCard, nextRoomCardIdSuffix); 
            roomsGridContainer.appendChild(newRoomCard); nextRoomCardIdSuffix++; updateTotalSurface();
        });
    }

    if (roomsGridContainer) { /* ... (Listener input roomsGridContainer come prima) ... */
        roomsGridContainer.addEventListener('input', function(e) {
            if (e.target.classList.contains('roomSurface')) { updateTotalSurface(); }
        });
    }
    
    function updateDynamicSelectsBasedOnBrandAndPanel() { /* ... (Implementazione COMPLETA come fornita nella risposta precedente, che popola panelModelSelect e generalPipePitchSelect) ... */
        ...
        generalPipePitchSelect.innerHTML = '<option value="10">10 cm</option><option value="15" selected>15 cm</option><option value="20">20 cm</option><option value="25">25 cm</option>'; // Aggiunto 25cm qui come fallback
            ...
            if (matchingPanels.length > 0) {
                ...
                panelModelSelect.onchange = function() {
                    ...
                    if (passiDisponibiliMm.length > 0) {
                        ...
                        if (passiDisponibiliMm.map(pVal => pVal / 10).includes(defaultPitchCm)) { generalPipePitchSelect.value = defaultPitchCm.toString(); }
                        else if (passiDisponibiliMm.length > 0 && !isNaN(passiDisponibiliMm[0]) && passiDisponibiliMm[0] > 0) { generalPipePitchSelect.value = (passiDisponibiliMm[0] / 10).toString(); }
                    } else { generalPipePitchSelect.innerHTML = '<option value="10">10 cm</option><option value="15" selected>15 cm</option><option value="20">20 cm</option><option value="25">25 cm</option>';}
                };
    ...
    loadThermolutzData().then(() => {
        ...
        }).catch(err => { console.error("Errore catena caricamento iniziale:", err); });
    ...
    if (calculateBtn) {
        calculateBtn.addEventListener('click', function() {
            ...
            } else if (selectedBrand === 'thermolutz' && thermolutzProducts.length > 0 ) { // Fallback
            ...
            // Impostazione spessore pannello e info
            if (foundThermolutzPanel) {
                ...
                if (foundThermolutzPanel.mq_per_confezione && foundThermolutzPanel.mq_per_confezione > 0) {
                ...
                if (passiDisponibili.length > 0) { panelInfoText += `<br><small>Passi disp. (mm): ${passiDisponibili.join('/')}</small>`; }
            ...
            rooms.forEach(room => {
                ...
                if (selectedBrand === 'thermolutz' && foundThermolutzPanel) {
                    ...
                    // Logica di interpolazione/estrapolazione per resa
                    ...
                    else if (currentRoomPipePitchCm > 20 && currentRoomPipePitchCm <= 25) { // Gestione per 25cm
                        if (r20 > 0 && r15 > 0 && r20 < r15) { // Se la resa diminuisce con passo maggiore (comportamento atteso)
                            const pendenza15_20 = (r15 - r20) / (20 - 15); // (W/mq)/cm 
                            resaTL = r20 - pendenza15_20 * (currentRoomPipePitchCm - 20); // Estrapola
                        } else if (r20 > 0) {
                            resaTL = r20 * 0.85; // Riduzione euristica se non si può estrapolare bene
                        } else {
                            resaTL = r15 * 0.7 || r10 * 0.6 || 35; // Fallback progressivo
                        }
                    } else { resaTL = r20 || r15 || r10 || 40; } // Fallback generale per altri passi
                    estimatedWattPerMq = Math.max(20, resaTL); // Minimo 20W/mq
            ...
            if (selectedBrand === 'thermolutz' && thermolutzProducts.length > 0) { /* ... Ricerca Tubo, Collettore, Accessori (come prima, usando chiavi corrette es. p.attacchi_collettore) ... */
                let tlPipeKeywords = ["TUBO"]; if (userPipeDiameter === '16x2') tlPipeKeywords.push("16X2", "16x2,0"); else if (userPipeDiameter === '17x2') tlPipeKeywords.push("17X2", "17x2,0"); const foundPipe = thermolutzProducts.find(p => p.descrizione && tlPipeKeywords.some(kw => p.descrizione.toUpperCase().includes(kw)) && (p.descrizione.toUpperCase().includes("PE-XA")||p.descrizione.toUpperCase().includes("PERT")||p.descrizione.toUpperCase().includes("MULTISTRATO")) && (p.descrizione.toUpperCase().includes("ROTOL")||p.descrizione.toUpperCase().includes(" MT")) && !p.descrizione.toUpperCase().includes("RACCORDO")); if(foundPipe){document.getElementById('thermolutzPipeInfo').innerHTML = `<li>TL Tubo: ${foundPipe.codice_fornitore || foundPipe.id} - ${foundPipe.descrizione.substring(0,60)}...</li>`; const match = foundPipe.descrizione.match(/(\d+)\s*MT/i); if(match && match[1]){actualPipeRollLength = parseInt(match[1]);}else if(foundPipe.imballo && typeof foundPipe.imballo === 'number' && foundPipe.imballo > 50){actualPipeRollLength = foundPipe.imballo;}}else{document.getElementById('thermolutzPipeInfo').innerHTML = `<li>TL Tubo (${userPipeDiameter}): Non trovato.</li>`;}
                const foundCollector = thermolutzProducts.find(p => p.descrizione && p.descrizione.toUpperCase().includes("COLLETTORE") && parseInt(p.attacchi_collettore) === estimatedTotalCircuits && !p.descrizione.toUpperCase().includes("CASSETTA")); if(foundCollector){document.getElementById('thermolutzCollectorInfo').innerHTML = `<li>TL Collettore (${estimatedTotalCircuits} vie): ${foundCollector.codice_fornitore || foundCollector.id} - ${foundCollector.descrizione.substring(0,60)}...</li>`;}else{const genericCollector = thermolutzProducts.find(p => p.descrizione && p.descrizione.toUpperCase().includes("COLLETTORE") && parseInt(p.attacchi_collettore) >= estimatedTotalCircuits); if(genericCollector){document.getElementById('thermolutzCollectorInfo').innerHTML = `<li>TL Collettore: ${genericCollector.codice_fornitore || genericCollector.id} - ${genericCollector.descrizione.substring(0,60)}... (Vie disp: ${genericCollector.attacchi_collettore}, Nec: ${estimatedTotalCircuits})</li>`;} else {document.getElementById('thermolutzCollectorInfo').innerHTML = `<li>TL Collettore: Non trovato.</li>`;}}
            ...
            // Aggiornamento output DOM finale
            ...
            document.getElementById('pipePitchOutput').textContent = userGeneralPipePitchCm.toFixed(1); // Ora usa userGeneralPipePitchCm
            ...
            let materialiAggiuntiviHTML = "<h3>Materiali Aggiuntivi (Stima):</h3><ul>"; const materialiSpecs = [{cod:'87288',descDefault:'Foglio Protettivo',coeff:1.1,unita:'mq'},{cod:'174454',descDefault:'Isolcasa Lastre',coeff:1.1,unita:'mq'},{cod:'60303',descDefault:'Additivo Fluidificante',coeff:0.20,unita:'kg'},{cod:'136402',descDefault:'Clips Fissaggio',coeff:1,unita:'pz (stima x mq)'},{cod:'265982',descDefault:'Tubo Corrugato',coeff:1,unita:'mt'}];
            materialiSpecs.forEach(spec => { const qty=totalRadiantSurface*spec.coeff;const p=thermolutzProducts.find(pr=>pr.codice_fornitore===spec.cod||pr.articolo_cai===spec.cod);const d=p?(p.descrizione||spec.descDefault):spec.descDefault;const c=p?(p.codice_fornitore||p.id||spec.cod):spec.cod;materialiAggiuntiviHTML+=`<li>${d} (Art. ${c}): ${qty.toFixed(spec.unita==='kg'||spec.unita==='mt'?1:2)} ${spec.unita}</li>`;}); materialiAggiuntiviHTML+="</ul><p><small>Quantità indicative.</small></p>";
