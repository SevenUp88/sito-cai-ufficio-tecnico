...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // MODIFICA (A) in processLoadedData per outdoorUnits
    function processLoadedData(brandsDocs, configTypesDocs, seriesMapDocs, outdoorUnitsDocs, indoorUnitsDocs) {
        ...
        APP_DATA.outdoorUnits = outdoorUnitsDocs.map((ue_doc, index) => { 
            ...
            const connections = Number(ue_doc.unit_collegabili) || 0; // ESATTO numero di UI supportate
            // minConnections non è più usato se connections è esatto. Se vuoi un controllo, puoi tenerlo
            // const minConnections = Number(ue_doc.min_connessioni_ue) || connections; // O imposta a connections se vuoi exact match
            ...
            return { 
                ...
                kw: uePotenzaKw, 
                connections: connections, // Campo principale per il numero di UI
                // minConnections: minConnections, // Potrebbe essere rimosso o impostato uguale a connections
    ...
    // --- UI Element Creation Helper Functions (copiate dal tuo script (1).js) ---
    ...
    // MODIFICA (B) in populateConfigTypes
    function populateConfigTypes() { 
        ...
        const validConfigs = Object.values(APP_DATA.configTypes).filter(ct => 
            APP_DATA.outdoorUnits.some(ue => 
                ...
                ue.connections === ct.numUnits // ESATTA CORRISPONDENZA
            )
    ...
    // MODIFICA (C) in populateIndoorSeries
    function populateIndoorSeries() {
        ...
        const candidateUEs = APP_DATA.outdoorUnits.filter(ue => 
            ue.brandId === brandId &&
            ue.connections === numUnitsRequired // ESATTA CORRISPONDENZA
        );
    ...
    // MODIFICA (D) in populateOutdoorUnits
    function populateOutdoorUnits() { 
        outdoorUnitSelectionDiv.innerHTML = ''; 
        if (!selections.brand || !selections.configType || !selections.indoorSeries) { outdoorUnitSelectionDiv.innerHTML = '<p>Scegli Marca, Config., Modello.</p>'; return; }
        ...
        const compatibleUEs = APP_DATA.outdoorUnits.filter(ue => 
            ue.brandId === selections.brand.id && 
            ue.connections === numRequired && // ESATTA CORRISPONDENZA
            Array.isArray(ue.compatibleIndoorSeriesIds) && 
            ue.compatibleIndoorSeriesIds.includes(requiredSeriesId)
        );
        if (!compatibleUEs.length) { outdoorUnitSelectionDiv.innerHTML = `<p>Nessuna UE ${selections.brand.name} esattamente per ${numRequired} UI e compatibile con Modello "${selections.indoorSeries.name}".</p>`; return; }
        console.log(`DEBUG: Found ${compatibleUEs.length} compatible UEs for exact ${numRequired} UI and series ${requiredSeriesId}.`);
        compatibleUEs.forEach(ue => { outdoorUnitSelectionDiv.appendChild(createUnitSelectionCard(ue, (selectedUE) => { if (selections.outdoorUnit?.id !== selectedUE.id) { resetSelectionsAndUIFrom(5); selections.outdoorUnit = selectedUE; highestLogicalStepCompleted = 4; } populateIndoorUnitSelectors(); showStep(5); }, selections.outdoorUnit?.id === ue.id)); }); 
...
// Initialize Firebase
const app = initializeApp(firebaseConfig);("DOM Contenuto Caricato - Inizio script.js (6-Step Flow - V2.1 Complete)")
