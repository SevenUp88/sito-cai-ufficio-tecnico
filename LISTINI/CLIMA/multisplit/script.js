// --- START OF SCRIPT.JS (FIREBASE - 6 STEP FLOW - COMPLETE WITH HELPERS) ---
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    function processLoadedData(brandsDocs, configTypesDocs, seriesMapDocs, outdoorUnitsDocs, indoorUnitsDocs) {
        ...
        APP_DATA.outdoorUnits = outdoorUnitsDocs.map((ue_doc, index) => {
            const brandId = String(ue_doc.marca || 'sconosciuta').toLowerCase(); const connections = Number(ue_doc.unit_collegabili) || 0; const minConnectionsFallback = connections > 0 ? (connections < 2 ? 1 : 2) : 1; const uePotenzaKw = Number(ue_doc.potenza) || 0; const coolingBTU_UE = Math.round(uePotenzaKw * 3412.14); const heatingBTU_UE = coolingBTU_UE;
            return { id: ue_doc.id || `ue_${index}`, brandId: brandId, modelCode: ue_doc.codice_prodotto || "N/A", name: ue_doc.nome_modello_ue && ue_doc.nome_modello_ue !== "Dati mancanti" ? `${String(ue_doc.marca || '').toUpperCase()} ${ue_doc.nome_modello_ue}` : `UE ${String(ue_doc.marca || '').toUpperCase()} (${ue_doc.codice_prodotto || 'ID: ' + ue_doc.id})`, connections: connections, minConnections: Number(ue_doc.min_connessioni_ue) || minConnectionsFallback, capacityCoolingBTU: coolingBTU_UE, capacityHeatingBTU: heatingBTU_UE, price: Number(ue_doc.prezzo) || 0, dimensions: ue_doc.dimensioni_ue || "N/A", weight: (ue_doc.peso_ue !== "Dati mancanti" && ue_doc.peso_ue !== undefined) ? ue_doc.peso_ue : "N/D", energyClassCooling: ue_doc.classe_energetica_raffrescamento || "N/D", energyClassHeating: ue_doc.classe_energetica_riscaldamento || "N/D", compatibleIndoorSeriesIds: Array.isArray(ue_doc.compatibleIndoorSeriesIds) ? ue_doc.compatibleIndoorSeriesIds : [] };
        });
    ...
    // --- UI Element Creation Helper Functions (MUST BE PRESENT) ---
    ...
    function createSelectionItem(item, type, clickHandler, isSelected = false) {
        ...
        itemDiv.dataset[type + 'Id'] = item.id; // Use item.id (which might be brand id, config id, series id)
        
        let logoSrc = ''; if (type === 'brand' && item.logo) logoSrc = item.logo;
        const nameSpan = document.createElement('span'); nameSpan.textContent = item.name; // item.name (brand name, config name, series name)
        ...
        itemDiv.addEventListener('click', () => {
            ...
            clickHandler(item); // Pass the full item object (which includes id, name, logo/numUnits etc)
        }); 
        return itemDiv;
    }
   ...
   function createUnitSelectionCard(unit, clickHandler, isSelected = false) {
    ...
    function populateConfigTypes(restoring = false) {
        ...
        if (validConfigs.length === 0) { configTypeSelectionDiv.innerHTML = `<p>Nessuna config. trovata per UE ${selections.brand.name}.</p>`; if (restoring) selections.configType = null; return; }
    ...
    function populateIndoorSeries(restoring = false) {
        ...
        const uniqueSeries = []; const seenSeriesNames = new Set(); validIndoorUnitsForSeriesSelection.forEach(ui => { if (!seenSeriesNames.has(ui.seriesName)) { uniqueSeries.push({ name: ui.seriesName, id: ui.seriesId }); seenSeriesNames.add(ui.seriesName); } });
    ...
    function populateOutdoorUnits(restoring = false) {
        outdoorUnitSelectionDiv.innerHTML = ''; if (!selections.brand || !selections.configType || !selections.indoorSeries) { outdoorUnitSelectionDiv.innerHTML = '<p>Seleziona Marca, Config., Serie.</p>'; if(restoring) selections.outdoorUnit = null; return; }
        ...
        console.log(`DEBUG: Found ${compatibleUEs.length} compatible UEs for series ${requiredSeriesId}.`);
        if (compatibleUEs.length === 0) { outdoorUnitSelectionDiv.innerHTML = `<p>Nessuna UE ${selections.brand.name} per config. ${selections.configType.name} compatibile con Serie "${selections.indoorSeries.name}".</p>`; if(restoring) selections.outdoorUnit = null; return; }
    ...
    function populateIndoorUnitSelectors(restoring = false) { 
        ...
        if (availableIndoorUnitsForSeries.length === 0) { indoorUnitsSelectionArea.innerHTML = `<p>Nessuna Unità Interna trovata per la serie ${selections.indoorSeries.name}.</p>`; checkAllIndoorUnitsSelected(); return; }
        console.log(`DEBUG: Units available for series ${selections.indoorSeries.name}: ${availableIndoorUnitsForSeries.length}`);
        ...
        for (let i = 0; i < selections.configType.numUnits; i++) {
            ...
            const label = document.createElement('label'); label.htmlFor = `indoor-unit-select-${i}`; label.textContent = `Unità Interna ${i + 1} (Serie: ${selections.indoorSeries.name}):`; label.style.cssText = 'display: block; margin-bottom: 5px; font-weight: bold;'; slotDiv.appendChild(label);
            ...
            const placeholder = document.createElement('option'); placeholder.value = ""; placeholder.textContent = "-- Seleziona Taglia/Modello --"; select.appendChild(placeholder);
            availableIndoorUnitsForSeries.forEach(uiVariant => { const option = document.createElement('option'); option.value = uiVariant.id; option.textContent = `${uiVariant.name} (Cod: ${uiVariant.modelCode} Prezzo: ${uiVariant.price.toFixed(2)}€)`; if (validRestore && selections.indoorUnits[i] && selections.indoorUnits[i].id === uiVariant.id) { option.selected = true; } select.appendChild(option); });
            ...
            if (validRestore && selections.indoorUnits[i]) { const currentUI = selections.indoorUnits[i]; detailsDiv.innerHTML = `<p style="margin: 3px 0;"><strong>Modello Selezionato:</strong> ${currentUI.modelCode}</p><p style="margin: 3px 0;"><strong>Potenza:</strong> ${currentUI.kw}kW (${currentUI.capacityBTU} BTU) - <strong>Prezzo:</strong> ${currentUI.price.toFixed(2)}€</p>${currentUI.image ? `<img src="${currentUI.image}" alt="${currentUI.name}" class="ui-details-img" style="max-width: 150px; max-height: 100px; object-fit: contain; margin-top: 5px;">` : ''}`; }
            select.addEventListener('change', (e) => { const selectedId = e.target.value; const unitIndex = parseInt(e.target.dataset.index); const selectedUIVariant = availableIndoorUnitsForSeries.find(u => u.id === selectedId); selections.indoorUnits[unitIndex] = selectedUIVariant || null; if (selectedUIVariant) { const currentUI = selectedUIVariant; detailsDiv.innerHTML = `<p style="margin: 3px 0;"><strong>Modello Selezionato:</strong> ${currentUI.modelCode}</p><p style="margin: 3px 0;"><strong>Potenza:</strong> ${currentUI.kw}kW (${currentUI.capacityBTU} BTU) - <strong>Prezzo:</strong> ${currentUI.price.toFixed(2)}€</p>${currentUI.image ? `<img src="${currentUI.image}" alt="${currentUI.name}" class="ui-details-img" style="max-width: 150px; max-height: 100px; object-fit: contain; margin-top: 5px;">` : ''}`; } else { detailsDiv.innerHTML = ''; } checkAllIndoorUnitsSelected(); });
    ...
    const seriesChanged = (prev, current) => (!prev && current) || (prev && current && prev.id !== current.id); // New check needed? Maybe not if ID sufficient
    ...
    function showStep(logicalStepNumber, fromDirectNavigation = false) {
        ...
        if (fromDirectNavigation && logicalStepNumber <= highestLogicalStepCompleted + 1) { clearFutureSelections(logicalStepNumber, true); } // Clear steps AFTER target, keep target step data
        updateStepIndicator(); window.scrollTo(0, 0);
    }

    function clearFutureSelections(targetLogicalStep, preserveTargetLevelSelections = false) {
        // targetLogicalStep = The step number we are NAVIGATING TO. Clear steps *after* this one.
        console.log(`clearFutureSelections called. Target step: ${targetLogicalStep}, Preserve: ${preserveTargetLevelSelections}`);
        let changed = false;

        // If target is Step 1 (Marca) or resetting all: Clear everything from Step 2 (Config) onwards
        if (targetLogicalStep < 2) { 
            if(selections.configType) { selections.configType = null; changed = true; }
            if(selections.indoorSeries) { selections.indoorSeries = null; changed = true; }
            if(selections.outdoorUnit) { selections.outdoorUnit = null; changed = true; }
            if(selections.indoorUnits.length > 0) { selections.indoorUnits = []; changed = true; }
        }
        // If target is Step 2 (Config) or resetting after Brand change: Clear from Step 3 (Series) onwards
        if (targetLogicalStep < 3 && !preserveTargetLevelSelections || targetLogicalStep < 2 && preserveTargetLevelSelections) { 
            if(selections.indoorSeries) { selections.indoorSeries = null; changed = true; }
            if(selections.outdoorUnit) { selections.outdoorUnit = null; changed = true; }
            if(selections.indoorUnits.length > 0) { selections.indoorUnits = []; changed = true; }
        }
        // If target is Step 3 (Series): Clear from Step 4 (UE) onwards
         if (targetLogicalStep < 4 && !preserveTargetLevelSelections || targetLogicalStep < 3 && preserveTargetLevelSelections) { 
            if(selections.outdoorUnit) { selections.outdoorUnit = null; changed = true; }
            if(selections.indoorUnits.length > 0) { selections.indoorUnits = []; changed = true; }
        }
        // If target is Step 4 (UE): Clear Step 5 (UI Sizes)
        if (targetLogicalStep < 5 && !preserveTargetLevelSelections || targetLogicalStep < 4 && preserveTargetLevelSelections) { 
             if(selections.indoorUnits.length > 0) { selections.indoorUnits = []; changed = true; }
        }
        // If target is Step 5 (UI Sizes): Clear nothing further (summary handled separately)

        if (changed || !preserveTargetLevelSelections) { // Only repopulate/clear UI if data actually changed or full clear requested
             if (!selections.brand) { configTypeSelectionDiv.innerHTML = '<p>Seleziona marca.</p>'; indoorSeriesSelectionDiv.innerHTML = '<p>Seleziona marca.</p>'; outdoorUnitSelectionDiv.innerHTML = '<p>Seleziona marca.</p>'; indoorUnitsSelectionArea.innerHTML = '<p>Seleziona marca.</p>';}
             else if (!selections.configType) { configTypeSelectionDiv.innerHTML = '<p>Caricamento...</p>'; populateConfigTypes(); indoorSeriesSelectionDiv.innerHTML = '<p>Seleziona config.</p>'; outdoorUnitSelectionDiv.innerHTML = '<p>Seleziona config.</p>'; indoorUnitsSelectionArea.innerHTML = '<p>Seleziona config.</p>';}
             else if (!selections.indoorSeries) { indoorSeriesSelectionDiv.innerHTML = '<p>Caricamento...</p>'; populateIndoorSeries(); outdoorUnitSelectionDiv.innerHTML = '<p>Seleziona serie.</p>'; indoorUnitsSelectionArea.innerHTML = '<p>Seleziona serie.</p>'; }
             else if (!selections.outdoorUnit) { outdoorUnitSelectionDiv.innerHTML = '<p>Caricamento...</p>'; populateOutdoorUnits(); indoorUnitsSelectionArea.innerHTML = '<p>Seleziona UE.</p>'; }
             else if (selections.indoorUnits.length === 0 && selections.configType?.numUnits > 0) { indoorUnitsSelectionArea.innerHTML = '<p>Caricamento...</p>'; populateIndoorUnitSelectors(); }
             // Summary clear
             if (targetLogicalStep <= TOTAL_LOGICAL_STEPS -1) { summaryDiv.innerHTML = ''; document.getElementById('summary-main-title')?.classList.remove('print-main-title'); }
              // Reset completion only if clearing beyond target or resetting all
             if (!preserveTargetLevelSelections && targetLogicalStep > 0) highestLogicalStepCompleted = Math.min(highestLogicalStepCompleted, targetLogicalStep - 1);
             else if (!preserveTargetLevelSelections && targetLogicalStep === 0) highestLogicalStepCompleted = 0; // full reset case
        }
        checkAllIndoorUnitsSelected(); updateStepIndicator();
    }
    ...
    function checkAllIndoorUnitsSelected() { 
        ...
        if(allSelected) { highestLogicalStepCompleted = Math.max(highestLogicalStepCompleted, 5); } // UI selection (step 5) complete
        updateStepIndicator(); 
    }
...
// --- END OF SCRIPT.JS (FIREBASE - 6 STEP FLOW - COMPLETE WITH HELPERS) ---
