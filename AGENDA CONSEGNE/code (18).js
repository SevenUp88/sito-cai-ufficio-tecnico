...
function loadGoogleMapsScript() {
    ...
    script.src = `https://maps.googleapis.com/maps/api/js?key=AIzaSyC6tvhoIlvIyh8L_jwSVWs_TkXNLKrt540&libraries=places`; // La tua chiave
    script.async = true;
    script.defer = true;
...
async function setupRealtimeListener() {
    ...
    unsubscribe = onSnapshot(docRef, (docSnap) => {
        ...
        buildDayDetailView(currentDayKey, false); // Non scrollare la vista ad ogni update
    });
}
...
function handleAutoSave(event) {
    ...
    const [dayKey, hour, field] = event.target.id.split('-');
    if (!dayKey || !hour || !field) return;
    
    agendaData[`${dayKey}-${hour}-${field}`] = event.target.value.trim();
...
function initAutocomplete(dayKey) {
    ...
    allHours.forEach(hour => {
        const input = document.getElementById(`${dayKey}-${hour}-indirizzo`);
...
function deleteSingleSlot(event) {
    ...
    if (confirm(`Sei sicuro di voler cancellare la consegna delle ${hour} di ${dayLabels[day]}?`)) {
        ...
        clearTimeout(saveTimeout);
        handleAutoSave({ target: { id: `${day}-${hour}-fake` } }); // Forza il salvataggio dei dati cancellati
    }
}
