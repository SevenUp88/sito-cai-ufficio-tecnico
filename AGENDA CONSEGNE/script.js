...
const ICONS = { /* ... Contenuto invariato ... */ };
...
function createCardHTML(date, hour, note, indirizzo, contatto) {
    const dataKey = `${date}-${hour}`;
    return `<div class="delivery-card ${note || indirizzo ? 'is-filled' : ''}" id="card-${dataKey}">
                <div class="card-header"><div class="card-time">${hour}</div></div>
                <div class="form-group">${ICONS.note}<textarea id="${dataKey}-note" placeholder="Note consegna...">${note}</textarea></div>
                <div class="form-group address-group">${ICONS.address}<input type="text" id="${dataKey}-indirizzo" value="${indirizzo}" placeholder="Indirizzo"></div>
